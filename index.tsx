document.addEventListener('DOMContentLoaded', () => {
    ...
    const pllAlgorithms = [
        ...
        { name: 'Ga', group: 'G Perms', algorithm: "R2 u R' U R' U' R u' R2 y' R' U R", image: "https://in.speedcube.com.au/cdn/shop/articles/ga_51fae21c-4fed-466e-88bf-96518730a0c2_200x.png?v=1704584305" },
        { name: 'Gb', group: 'G Perms', algorithm: "F' U' F R2 u R' U R U' R u' R2'", image: "https://in.speedcube.com.au/cdn/shop/articles/gb_5cde8f32-3565-45b3-839c-e8a064a2c6a3_200x.png?v=1704585973" },
        { name: 'Gc', group: 'G Perms', algorithm: "R2 U' R U' R U R' U R2 D' U R U' R' D U'", image: "https://in.speedcube.com.au/cdn/shop/articles/gc_5612f446-6915-462a-a894-84a31fa4bd39_200x.png?v=1704584825" },
        { name: 'Gd', group: 'G Perms', algorithm: "R U R' y' R2 u' R U' R' U R' u R2", image: "https://in.speedcube.com.au/cdn/shop/articles/gd_b09506e7-dfe4-4fa9-8c5e-5d559eacd829_200x.png?v=1704585507" },
    ...
    const ollAlgorithms = [
        ...
        { case: 3, name: "Sune/Anti-Sune", algorithm: "f R U R' U' f' U' F R U R' U' F'", image: "https://in.speedcube.com.au/cdn/shop/articles/oll3_0bb55b60-bcde-4427-b2d6-595f01540405_200x.png?v=1704584434" },
        { case: 4, name: "Sune/Anti-Sune", algorithm: "f R U R' U' f' U F R U R' U' F'", image: "https://in.speedcube.com.au/cdn/shop/articles/oll4_11e77a19-0f55-4827-87f3-8de35f2d30ac_200x.png?v=1704586335" },
    ...
    // --- State ---
    let activeSet = 'PLL';
    let learnedPLL = new Set();
    let learnedOLL = new Set();

    // --- DOM Elements ---
    const pllButton = document.getElementById('pll-btn');
    const ollButton = document.getElementById('oll-btn');
    ...
    const loadProgress = () => {
        const pllData = localStorage.getItem('learnedPLL');
        const ollData = localStorage.getItem('learnedOLL');
        if (pllData) {
            learnedPLL = new Set(JSON.parse(pllData));
        }
        if (ollData) {
            learnedOLL = new Set(JSON.parse(ollData));
        }
    };

    const saveProgress = () => {
        localStorage.setItem('learnedPLL', JSON.stringify(Array.from(learnedPLL)));
        localStorage.setItem('learnedOLL', JSON.stringify(Array.from(learnedOLL)));
    };

    const updateProgressUI = () => {
        const isPLL = activeSet === 'PLL';
        const learnedCount = isPLL ? learnedPLL.size : learnedOLL.size;
        const totalCount = isPLL ? pllAlgorithms.length : ollAlgorithms.length;
        const percentage = totalCount > 0 ? (learnedCount / totalCount) * 100 : 0;

        progressTitle.textContent = `${activeSet} Progress`;
        progressText.textContent = `${learnedCount} / ${totalCount} Learned`;
        progressBarFill.style.width = `${percentage}%`;
    };

    const handleCheckboxChange = (id, type, isChecked) => {
        const targetSet = type === 'PLL' ? learnedPLL : learnedOLL;
    ...
    const createAlgorithmCardHTML = (data, type) => {
        const isOLL = type === 'OLL';
        const id = isOLL ? data.case : data.name;
        const title = isOLL ? `OLL Case ${data.case}` : data.name;
        const subtitle = isOLL ? data.name : data.group;
        const isLearned = (isOLL ? learnedOLL : learnedPLL).has(id);

        const imageHTML = data.image 
            ? `<img src="${data.image}" alt="${title} diagram" class="case-image" loading="lazy" />`
            : `<p class="case-placeholder">Visual</p>`;

        return `
            <div class="card ${isLearned ? 'learned' : ''}" data-id="${id}" data-type="${type}">
                <label class="learned-checkbox" aria-label="Mark as learned">
                ...
                <div class="card-content">
                    ...
                    <p class="algorithm">${data.algorithm}</p>
                </div>
            </div>
        `;
    };
    ...
    const renderAlgorithms = () => {
        const isPLL = activeSet === 'PLL';
        const algorithmsToDisplay = isPLL ? pllAlgorithms : ollAlgorithms.sort((a, b) => a.case - b.case);
        const groupingKey = isPLL ? 'group' : 'name';

        const groupedAlgs = algorithmsToDisplay.reduce((acc, alg) => {
            const key = alg[groupingKey];
            if (!acc[key]) {
                acc[key] = [];
            }
            acc[key].push(alg);
        ...
        for (const groupName in groupedAlgs) {
            const groupHeader = document.createElement('h2');
            groupHeader.className = 'group-header';
            groupHeader.textContent = groupName;
            grid.appendChild(groupHeader);
            
            const groupAlgsHTML = groupedAlgs[groupName]
                .map(alg => createAlgorithmCardHTML(alg, activeSet))
                .join('');
            grid.insertAdjacentHTML('beforeend', groupAlgsHTML);
        }
        ...
        document.querySelectorAll('.learned-checkbox input').forEach(checkbox => {
            checkbox.addEventListener('change', (event) => {
                ...
                if (card) {
                    ...
                    if(id && type) {
                        const identifier = type === 'OLL' ? parseInt(id) : id;
                        handleCheckboxChange(identifier, type, isChecked);
                    }
                }
            });
        });
    ...
    const setActive = (newSet) => {
        if (activeSet === newSet) return;
        
        activeSet = newSet;

        if (newSet === 'PLL') {
            pllButton.classList.add('active');
            pllButton.setAttribute('aria-pressed', 'true');
            ollButton.classList.remove('active');
            ollButton.setAttribute('aria-pressed', 'false');
            // FIX: Property 'dataset' does not exist on type 'Element'. Cast to HTMLElement.
            (toggleContainer as HTMLElement).dataset.active = 'PLL';
        } else {
            ollButton.classList.add('active');
            ollButton.setAttribute('aria-pressed', 'true');
            pllButton.classList.remove('active');
            pllButton.setAttribute('aria-pressed', 'false');
            // FIX: Property 'dataset' does not exist on type 'Element'. Cast to HTMLElement.
            (toggleContainer as HTMLElement).dataset.active = 'OLL';
        }
    ...
    // --- Event Listeners ---
    pllButton.addEventListener('click', () => setActive('PLL'));
    ollButton.addEventListener('click', () => setActive('OLL'));
